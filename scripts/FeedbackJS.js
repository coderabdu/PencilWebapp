...
function validateUsername() {
    ...
    try {
        ...
        if (/.{4,}/.test(unInput.value) === false) {
            throw "Username must be at least 4 chracters long";
        } else if (/\w/.test(unInput.value) === true) {
            throw "Please must contain only letters and numbers";
        }
...
function validatePassword() {
    ...
    try {
        ...
        if (/.{8,}/.test(pw1Input.value) === false) {
            ...
            throw "Password must contain at least one of thefollowing symbols: ! @ # _";
        }
...
function convertToString() {
    // convert loding array to string
